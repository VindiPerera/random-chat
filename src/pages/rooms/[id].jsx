...
export default function Room() {
    ...
    useEffect(() => {
        ...
        const handleFiles = async (fileInput) => {
            ...
            if (!selectedFile.type || !selectedFile.type.startsWith('image/') || !validImageTypes.includes(selectedFile.type)) {
                setErrorMessage('Can\'t send file, only images are allowed!');
                setShowError(true);
            ...
            if (selectedFile.size > 10 * 1024 * 1024) {
                setErrorMessage('Image file too large. Maximum size is 10MB.');
                setShowError(true);
                fileInput.target.value = '';
                setTimeout(() => {
                    setShowError(false);
            ...
            // Only set file state if it's a valid image
            try {
        ...
        <div className={showMembers ? `2xl:grid grid-cols-12 relative` : `relative`}>
            <div className="col-span-9 2xl:w-full relative">
                ...
                <form onSubmit={e => {
                        ...
                        if((!message || message.trim().length === 0) && !file) return;
                        
                        // Don't send if file exists but it's not an image
                        ...
                        if ((message && message.trim().length > 0) && !file) {
                            connection.emit('message', { message: message.trim() });
                            e.target.message.value = '';
                        } else if (file && filetype && filetype.startsWith('image/')) {
                            connection.emit('message', { message: message?.trim() || '', file: file, type: filetype });
                            e.target.message.value = '';
                            setFile(null);
                            setFiletype(null);
                            setPreview(null);
                      ...
                      <motion.li className="flex flex-col space-y-4" layout initial='initial' animate='animate' exit='animate'> 
                        ...
                        <div className="flex items-center">
                            ...
                            <input  id="file" type="file" accept="image/*" onChange={handleFiles} className="h-full w-full text-white mx-1 absolute opacity-0"></input>
                                </div>
                            </button>
